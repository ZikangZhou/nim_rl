...
int main() {
  ...
  NStepSarsaAgent n_step_sarsa_agent1(alpha, gamma, 2, epsilon, decay_factor);
  NStepSarsaAgent n_step_sarsa_agent2(alpha, gamma, 2, epsilon, decay_factor);
  NStepExpectedSarsaAgent
      n_step_expected_sarsa_agent1(alpha, gamma, 2, epsilon, decay_factor);
  NStepExpectedSarsaAgent
      n_step_expected_sarsa_agent2(alpha, gamma, 2, epsilon, decay_factor);
  OffPolicyNStepSarsaAgent
      off_policy_n_step_sarsa_agent1(alpha, gamma, 2, epsilon, decay_factor);
  OffPolicyNStepSarsaAgent
      off_policy_n_step_sarsa_agent2(alpha, gamma, 2, epsilon, decay_factor);
  OffPolicyNStepExpectedSarsaAgent
      off_policy_n_step_expected_sarsa_agent1(alpha, gamma, 2, epsilon,
                                              decay_factor);
  OffPolicyNStepExpectedSarsaAgent
      off_policy_n_step_expected_sarsa_agent2(alpha, gamma, 2, epsilon,
                                              decay_factor);
  NStepTreeBackupAgent
      n_step_tree_backup_agent1(alpha, gamma, 2, epsilon, decay_factor);
  NStepTreeBackupAgent
      n_step_tree_backup_agent2(alpha, gamma, 2, epsilon, decay_factor);

  game.SetFirstPlayer(&policy_iteration_agent);
  game.SetSecondPlayer(&optimal_agent);
  game.Train();
  std::cout << policy_iteration_agent.GetValues() << std::endl;
  ...
  game.SetFirstPlayer(&off_policy_n_step_sarsa_agent1);
  game.SetSecondPlayer(&off_policy_n_step_sarsa_agent2);
  game.Train(train_episodes);
  std::cout << off_policy_n_step_sarsa_agent1.GetValues() << std::endl;
  game.SetSecondPlayer(&optimal_agent);
  game.Play(play_episodes);

  game.SetFirstPlayer(&off_policy_n_step_expected_sarsa_agent1);
  game.SetSecondPlayer(&off_policy_n_step_expected_sarsa_agent2);
  game.Train(train_episodes);
  std::cout << off_policy_n_step_expected_sarsa_agent1.GetValues() << std::endl;
  game.SetSecondPlayer(&optimal_agent);
  game.Play(play_episodes);

  game.SetFirstPlayer(&n_step_tree_backup_agent1);
  game.SetSecondPlayer(&n_step_tree_backup_agent2);
  game.Train(train_episodes);
  std::cout << n_step_tree_backup_agent1.GetValues() << std::endl;
  game.SetSecondPlayer(&optimal_agent);
  game.Play(play_episodes);
  return 0;
}
